...
func regenerate_mesh(planet_data: PlanetData):
	...
	var _resolution: int = resolution - 1
	var num_vertices: int = resolution ** 2
	#var num_indices: int = _resolution ** 2 * 6
	
	vertex_array.resize(num_vertices)
	uv_array.resize(num_vertices)
	normal_array.resize(num_vertices)
	index_array.resize(num_vertices)
	...
	for y in range(resolution):
		for x in range(resolution):
			...
			var percent := Vector2(x, y) / _resolution
			var point_on_unit_cube: Vector3 = normal + (percent.x - 0.5) * 2 * axisA + (percent.y - 0.5) * 2 * axisB
			vertex_array[i] = point_on_unit_cube
			
			if x != _resolution and y != _resolution:
